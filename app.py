...
import os
import traceback
from pitch_downloader_direct import DirectPitchDownloader
...
def health_check():
    return jsonify({
        'status': 'healthy',
        'service': 'Pitch.com Downloader API - Direct Approach',
        'version': '3.4',
        'max_slides': MAX_SLIDES,
        'timeout': TIMEOUT,
        'features': [
            'Direct API access approach',
...
def limits():
    return jsonify({
        ...
        'note': 'Direct approach with minimal configuration and data extraction',
        'improvements': [
            'Direct API access attempts',
            'Minimal browser configuration',
            'Data extraction from page scripts',
            'Simplified detection methods',
            'Reduced complexity'
        ]
    })
...
def download_presentation():
    try:
        ...
        logger.info(f"Direct approach - max_slides: {max_slides}, timeout: {TIMEOUT}s")
        
        # Create direct downloader
        downloader = DirectPitchDownloader(max_slides=max_slides, timeout=TIMEOUT)
