...
import signal
import sys
from pitch_downloader_robust import RobustPitchDownloader
...
def health_check():
    return jsonify({
        'status': 'healthy',
        'service': 'Pitch.com Downloader API - Robust',
        'version': '2.2',
        'max_slides': MAX_SLIDES,
        'timeout': TIMEOUT,
        'features': [
            'Robust browser handling',
            'Graceful shutdown support',
            'Fixed slide count detection',
...
def download_presentation():
    try:
        ...
        logger.info(f"Robust approach - max_slides: {max_slides}, timeout: {TIMEOUT}s")
        
        # Create robust downloader
        downloader = RobustPitchDownloader(max_slides=max_slides, timeout=TIMEOUT)
