...
import signal
import sys
from pitch_downloader_simple_robust import SimpleRobustPitchDownloader
...
def health_check():
    return jsonify({
        'status': 'healthy',
        'service': 'Pitch.com Downloader API - Simple Robust',
        'version': '2.3',
...
def download_presentation():
    try:
        ...
        logger.info(f"Simple robust approach - max_slides: {max_slides}, timeout: {TIMEOUT}s")
        
        # Create simple robust downloader
        downloader = SimpleRobustPitchDownloader(max_slides=max_slides, timeout=TIMEOUT)
        
        # Run download with proper event loop handling
        try:
            # Create new event loop for this request
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(downloader.download_presentation(url, filename))
            loop.close()
