...
import os
import traceback
from pitch_downloader_selenium import SeleniumPitchDownloader
...
def health_check():
    return jsonify({
        'status': 'healthy',
        'service': 'Pitch.com Downloader API - Selenium Approach',
        'version': '3.7',
...
def limits():
    return jsonify({
        ...
        'note': 'Selenium WebDriver approach with ChromeDriver stealth mode',
        'improvements': [
            'Selenium WebDriver instead of Pyppeteer',
            'ChromeDriver stealth configuration',
            'Multiple URL strategies',
            'Realistic browser fingerprinting',
            'Better bot detection bypass'
        ]
...
def download_presentation():
    try:
        ...
        logger.info(f"Selenium approach - max_slides: {max_slides}, timeout: {TIMEOUT}s")
        
        # Create Selenium downloader
        downloader = SeleniumPitchDownloader(max_slides=max_slides, timeout=TIMEOUT)
