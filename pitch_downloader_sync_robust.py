...
class SyncRobustPitchDownloader:
    ...
    def navigate_to_presentation_sync(self, url: str) -> bool:
        """Navigate to presentation synchronously with simplified loading detection"""
        try:
            ...
            # Simple content check without complex waiting
            try:
                content_length = self.loop.run_until_complete(self.page.evaluate('() => document.body.innerText.length'))
    ...
    def capture_slide_sync(self, slide_number: int) -> Optional[bytes]:
        """Capture slide synchronously with simplified approach"""
        try:
            logger.info(f"Capturing slide {slide_number}")
            
            # Simple wait for slide to stabilize
            self.loop.run_until_complete(asyncio.sleep(3))
    ...
    def navigate_to_next_slide_sync(self) -> bool:
        """Navigate to next slide synchronously with simplified approach"""
        try:
    ...
    def download_presentation(self, url: str, filename: str) -> Dict:
        """Synchronous download method with signal handling"""
        try:
            import time
            import signal
