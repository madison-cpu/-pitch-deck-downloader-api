...
class SyncRobustPitchDownloader:
    ...
    def launch_browser_sync(self):
        ...
        try:
            ...
            # Create persistent event loop for all browser operations
            ...
            self.loop.run_until_complete(self.page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'))
    ...
    def navigate_to_presentation_sync(self, url: str) -> bool:
        """Navigate to presentation synchronously with better loading detection"""
        try:
            ...
            try:
                self.loop.run_until_complete(self.page.waitForFunction('''() => {
    ...
    def capture_slide_sync(self, slide_number: int) -> Optional[bytes]:
        """Capture slide synchronously with proper loading detection"""
        try:
            ...
            try:
                ...
                logger.info(f"Slide {slide_number} content loaded")
            except Exception as e:
                logger.warning(f"Content check timed out for slide {slide_number}: {e}")
    ...
    def navigate_to_next_slide_sync(self) -> bool:
        """Navigate to next slide synchronously with proper waiting"""
        try:
            ...
            try:
                ...
                logger.info("Next slide loaded")
            except Exception as e:
                logger.warning(f"Next slide loading check timed out: {e}")
    ...
    def download_presentation(self, url: str, filename: str) -> Dict:
        ...
        try:
            ...
            # Wait for first slide to be fully loaded
            try:
                self.loop.run_until_complete(self.page.waitForFunction('''() => {
                    const loadingElements = document.querySelectorAll(
                        '[class*="loading"], [class*="spinner"]'
                ...
                logger.warning(f"First slide loading check timed out: {e}")
            
            # Additional wait to ensure first slide is fully rendered
            self.loop.run_until_complete(asyncio.sleep(3))
